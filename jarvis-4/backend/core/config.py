...
class Settings(BaseSettings):
    ...
    # --- Gateway (API server) ---
    # Cross-device/remote access is deferred for now (tracked in
    # docs/future-features.md) — the gateway only accepts localhost
    # connections. gateway_host is force-corrected to "127.0.0.1" in
    # the validator below even if something else is set in .env, so
    # this can't be silently bypassed by an old config file.
    gateway_host: str = Field(default="127.0.0.1")
    gateway_port: int = Field(default=8000)
    # Every request (REST or WebSocket) must include this token, e.g.
    # as "Authorization: Bearer <token>". Auto-generated by the setup
    # wizard (backend/setup.py) — you shouldn't need to set this by hand.
    gateway_auth_token: str | None = Field(default=None)

    @field_validator("gateway_host")
    @classmethod
    def _force_localhost_only(cls, value: str) -> str:
        if value != "127.0.0.1":
            warnings.warn(
                f"GATEWAY_HOST='{value}' was set in config/.env, but cross-device "
                "access is currently disabled — forcing 127.0.0.1. This is "
                "on the roadmap; see docs/future-features.md.",
                stacklevel=2,
            )
            return "127.0.0.1"
        return value
